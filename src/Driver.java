...
public class Driver implements Runnable, MouseMotionListener {
    ...
    int xTest = random.nextInt(width);
    int yTest = random.nextInt(height);
    int xTest2 = random.nextInt(width);
    int yTest2 = random.nextInt(height);

    private final Canvas canvas;

    private final LinkedList<Line2D.Float> lines;
    ...
    private void render(){
        ...
        LinkedList<Line2D.Float> rays = calcRays(lines,mouseX,mouseY,100);
        for(Line2D.Float ray : rays){
            g.drawLine((int) ray.x1, (int) ray.y1, (int) ray.x2, (int) ray.y2);
        }

        g.setColor(Color.RED);
        LinkedList<Line2D.Float> rays2 = calcRays(lines,xTest,yTest,200);
        for(Line2D.Float ray2 : rays2){
            g.drawLine((int) ray2.x1, (int) ray2.y1, (int) ray2.x2, (int) ray2.y2);
        }
        LinkedList<Line2D.Float> rays3 = calcRays(lines,xTest2,yTest2,200);
        for(Line2D.Float ray2 : rays3){
            g.drawLine((int) ray2.x1, (int) ray2.y1, (int) ray2.x2, (int) ray2.y2);
        }
    ...
    private LinkedList<Line2D.Float> calcRays(LinkedList<Line2D.Float> lines, int x,int y, int resolution) {
        int maxDist = 1000;
    ...
    public static float getRayCast(float p0_x, float p0_y, float p1_x, float p1_y, float p2_x, float p2_y, float p3_x, float p3_y) {
        ...
        float e = -s2_x * s1_y + s1_x * s2_y;
        float s, t;
        s = (-s1_y * (p0_x - p2_x) + s1_x * (p0_y - p2_y)) / e;
        t = (s2_x * (p0_y - p2_y) - s2_y * (p0_x - p2_x)) /e;
